...
def retry_with_backoff(
    ...
    raise RuntimeError("retry_with_backoff exhausted without returning")
...
def run_assistant(
    ...
    while True:
        ...
        assistant_tokens = count_tokens(assistant_reply, model=OPENAI_MODEL)
        total_tokens += user_tokens + assistant_tokens

        cost_stats = estimate_cost(user_msg, assistant_reply, model=OPENAI_MODEL)
        total_cost += float(cost_stats["total_cost"])
